...
def download_func(d, m, y):
    ...
    date_str = f"{y}{m:02d}{d:02d}"  # Ensure two-digit month and day

    # Construct URL with formatted date
    url = f"https://acdisc.gesdisc.eosdis.nasa.gov/opendap/HAQAST/MERRA2_CNN_HAQAST_PM25.1/{y}/MERRA2_HAQAST_CNN_L4_V1.{date_str}.nc4.dap.nc?dap4.ce=/lat[0:1:360];/lon[0:1:575];/time[0:1:23];/MERRA2_CNN_Surface_PM25[0:1:23][0:1:360][0:1:575]"

    # Define output filename
    filename = f"MERRA2_PM25_{date_str}.nc"

    # NASA Earthdata Authentication Token (Ensure it's valid)
